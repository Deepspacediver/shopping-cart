...
jest.mock(
  "../CartItem/CartItem",
  () =>
    ({ id, name, amount, handleIncrement, handleDecrement, handleRemove }) =>
      (
...
describe("Shopping cart products display", () => {
  ...
  test("if remove button removes product", () => {
    render(
      <ContextWrapperProp stateProp={stateProp}>
        <CartItemContainer />
      </ContextWrapperProp>
    ...
    const yellowBirdField = screen.getAllByTestId("products")[1];
    const yellowRemoveBtn = within(yellowBirdField).getByRole("button", {
      name: "Remove",
    });
    expect(yellowBirdField).toBeInTheDocument();
    userEvent.click(yellowRemoveBtn);
    expect(yellowBirdField).not.toBeInTheDocument();
  });
});
